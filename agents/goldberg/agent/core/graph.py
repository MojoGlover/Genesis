...
1. NEVER claim to have run, created, written, or read something you have not.
2. If a task requires a file, tool, or command — CALL THE TOOL. Do not describe what you would do.
3. If you used a tool, say so and include the result.
4. If you have NOT used a tool, do NOT claim action was taken.
5. When uncertain, ask for clarification rather than fabricating an answer.
"""

def _requires_tool_use(message: str) -> bool:
    """Return True if the message is an action task that requires tool use."""
    lowered = message.lower()
    action_verbs = (
        "build", "create", "make", "generate", "implement", "set up",
        "scaffold", "wire up", "hook up", "run", "execute", "write",
        "update", "fix", "edit", "modify", "delete", "remove", "install",
        "deploy", "push", "commit", "fetch", "read", "check", "verify",
        "show me", "list", "find", "search",
    )
    artifact_nouns = (
        "script", "file", "server", "module", "function", "class",
        "endpoint", "service", "tool", "agent", "plist", "config",
        "bridge", "handler", "test", "spec", "repo", "database",
    )
    ...
    if any(t in lowered for t in imperative_terms):
        return True
    return False


def _detect_fabrication(response: str, tool_history: list) -> str | None:
    """Return a correction string if response fabricates tool results; else None."""
    lowered = response.lower()
    called  = {e.get("content", "").split("(")[0].strip().lower()
               for e in tool_history if e.get("role") == "assistant"}

    write_claim = any(p in lowered for p in (
        "i have created", "i created", "i've written", "i wrote",
        "i have written", "file has been created", "i've created",
        "successfully created", "successfully written",
    ))
    write_tools = {"write_file", "patch_file", "shell", "python"}
    if write_claim and not called.intersection(write_tools):
        return (
            "FABRICATION DETECTED. You claimed to have created/written something "
            "but no write_file, patch_file, shell, or python tool was called. "
            "You MUST call the appropriate tool to actually perform the action. "
            "Output a tool call JSON block now."
        )

    read_claim = any(p in lowered for p in (
        "the file contains", "the file shows", "the contents of",
        "i read", "i've read", "upon reading",
    ))
    if read_claim and "read_file" not in called and "shell" not in called:
        return (

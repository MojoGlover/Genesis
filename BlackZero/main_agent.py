...
async def main() -> None:
    ...
    plugops_url = os.environ.get(
        "PLUGOPS_URL",
        f"ws://localhost:9000/ws/{agent_id}"
    )
    ...
    # 8. Autonomous loops — task_loop by default, configurable in config.yaml
    #    autonomy.loops: [{type: task_loop, interval_seconds: 30}, ...]
    #    Loop types: task_loop, monitor_loop, cron_loop, heartbeat_loop
    from agent.core.loops import build_loops
    loops = build_loops(config, graph, data_dir, agent_name)

    logger.info(f"{agent_name} ready — reactive + autonomous ({len(loops)} loop(s)).")
